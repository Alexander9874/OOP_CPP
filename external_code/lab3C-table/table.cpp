...
{	
	...
	for (int i = 0; i < table1.m_n; i++) {
		res.m_table[i] = table1.m_table[i];
	}
	for (int i = table1.m_n; i < res.m_size; i++) {
		res.m_table[i] = table2.m_table[i - table1.m_n];
	}
	return res;
}
...
Table& Table::operator +=(const Table &table)
{
	...
	for (int i = this->m_n; i < table.m_n; i++) {
		this->m_table[i] = table.m_table[i - this->m_n];
	}
	return *this;
}
...
Table operator - (const Table &table1, const Table &table2) noexcept
{
	...
	for (int i = 0; i < table1.m_n; i++) {
		...
		if (!is_in_table) {
			res.m_table[res.get_n()] = table1.m_table[i];
